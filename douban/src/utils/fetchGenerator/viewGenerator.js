...
function generateViewWithFetch({
  ...
  const composeURL = (inputAPI, inputParams = {}) => {
    let requestPara = "";
  ...
  const mapStateToProps = (state, ownProps) => {
    const currState = pageName
      ? state[pageName][moduleName]
      : state[moduleName];
    return {
      API: API,
      isLoading:
        typeof currState.isLoading === "undefined" ? true : currState.isLoading,
  ...
  const mapDispatchToProps = (dispatch, ownProps) => {
    return {
      fetchByParams: params => {
        const url = composeURL(API, params);
